...
public sealed class MicrosoftRegistration : RegistrationBase
{
    ...
    public MicrosoftRegistration(MicrosoftRegistrator registrator, Type implementationType) : base(registrator,
        implementationType)
    {
        _registrator = registrator;
    }

    public MicrosoftRegistration(MicrosoftRegistrator registrator, object implementationInstance) : base(registrator,
        implementationInstance)
    {
        _registrator = registrator;
    }

    public MicrosoftRegistration(MicrosoftRegistrator registrator, Expression<Func<IResolver, object>> factory,
        Type implementationType) : base(registrator, factory, implementationType)
    {
        _registrator = registrator;
    }

    private static readonly MethodInfo GetRequiredServiceMethod =
        typeof(ServiceProviderServiceExtensions).GetMethods().FirstOrDefault(x =>
            x is
            {
    ...
    private static readonly MethodInfo GetRequiredKeyedServiceMethod = typeof(ServiceProviderKeyedServiceExtensions).GetMethods().FirstOrDefault(x =>
                                       x is
                                       {
    ...
    {
        var visitor = new ReplaceResolverExpressionVisitor<IServiceProvider>((x, y) =>
        {
             var actualMethod = GetRequiredServiceMethod.MakeGenericMethod(y.Method.ReturnType);
        ...
        {
            var actualMethod = GetRequiredKeyedServiceMethod.MakeGenericMethod(y.Method.ReturnType);
    ...
    public override IRegistrator RegisterImplementation()
    {
        ...
        if (ShouldOnlyRegisterIfNotRegistered)
        {
            foreach (var serviceType in ServiceTypes)
            {
                ...
                {
                    _registrator.Services.TryAdd(ServiceDescriptor.Describe(serviceType,
                        TranslateFactory(ImplementationFactory), ToMicrosoftLifetime(Lifetime.Value)));
                }
                else if (ImplementationInstance is not null)
                {
                    _registrator.Services.TryAdd(ServiceDescriptor.Describe(serviceType, _ => ImplementationInstance,
                        ToMicrosoftLifetime(Lifetime.Value)));
                }
                else if (ImplementationType is not null)
                {
                    _registrator.Services.TryAdd(ServiceDescriptor.Describe(serviceType, ImplementationType,
                        ToMicrosoftLifetime(Lifetime.Value)));
                }
            }
        }
        else
        {
            foreach (var serviceType in ServiceTypes)
            {
                if (ImplementationFactory is not null)
                {
                    _registrator.Services.Add(ServiceDescriptor.Describe(serviceType,
                        TranslateFactory(ImplementationFactory), ToMicrosoftLifetime(Lifetime.Value)));
                }
                else if (ImplementationInstance is not null)
                {
                    _registrator.Services.Add(ServiceDescriptor.Describe(serviceType, _ => ImplementationInstance,
                        ToMicrosoftLifetime(Lifetime.Value)));
                }
                else if (ImplementationType is not null)
                {
                    _registrator.Services.Add(ServiceDescriptor.Describe(serviceType, ImplementationType,
                        ToMicrosoftLifetime(Lifetime.Value)));
                }
            }
        }
    ...
    public static ServiceLifetime ToMicrosoftLifetime(AttributeBasedRegistration.ServiceLifetime lifetime)
        => lifetime switch
        {
            ...
            AttributeBasedRegistration.ServiceLifetime.InstancePerMatchingLifetimeScope =>
